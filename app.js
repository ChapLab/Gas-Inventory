...
function bindEvents(){
  ...
  on("addRoom","input",()=>updatePositionDatalistForRoom("addRoom","addPositionOptions"));
  on("addRoom","change",()=>updatePositionDatalistForRoom("addRoom","addPositionOptions"));
...
function populateFilters(){
  ...
  if(el("addPositionOptions"))updatePositionDatalistForRoom("addRoom","addPositionOptions");
}
...
function updatePositionDatalistForRoom(roomInputId,datalistId){
  const room=el(roomInputId)?el(roomInputId).value:"";
  if(el(datalistId))fillDatalist(el(datalistId),positionsForRoom(room));
}
...
function fillDatalist(list,values){
  list.innerHTML="";
  values.forEach(v=>{const option=document.createElement("option");option.value=v;list.appendChild(option);});
}
...
function renderKnownTankUpdate(t){
  ...
  el("scanResult").innerHTML=`
    <div class="card">
      ...
      <input id="updatePosition" value="${escapeAttr(t["Position"]||"")}" list="updatePositionOptions" placeholder="Example: Back wall rack" />
      <datalist id="updatePositionOptions"></datalist>
      <label>Status</label>
  ...
  updatePositionDatalistForRoom("updateRoom","updatePositionOptions");
  on("updateRoom","input",()=>updatePositionDatalistForRoom("updateRoom","updatePositionOptions"));
  on("updateRoom","change",()=>updatePositionDatalistForRoom("updateRoom","updatePositionOptions"));
  on("saveScannedUpdateBtn","click",async()=>{
    ...
    const updates={"Room":el("updateRoom").value.trim(),"Position":el("updatePosition").value.trim(),"Status":el("updateStatus").value,"Updated By":updatedBy};
    if(!updates["Room"]||!updates["Position"]){showToast("Room and position are required.");return;}
    locallyApplyUpdate(t["Barcode"],updates);showToast("Saving update...");
    try{await api("updateFull",{barcode:t["Barcode"],tank:updates});showToast("Tank updated.");await refreshData();el("scanResult").innerHTML=emptyState("Saved. Keep scanning or clear the scanner when done.");}
    catch(err){showToast(err.message);await refreshData();}
  });
...
function renderNewTankSetup(barcode){
  ...
  el("scanResult").innerHTML=`
    <div class="card warning">
      ...
      <label>Position</label>
      <input id="newPosition" list="newPositionOptions" placeholder="Example: Back wall rack" />
      <datalist id="newPositionOptions"></datalist>
      <label>Status</label>
  ...
  updatePositionDatalistForRoom("newRoom","newPositionOptions");
  on("newRoom","input",()=>updatePositionDatalistForRoom("newRoom","newPositionOptions"));
  on("newRoom","change",()=>updatePositionDatalistForRoom("newRoom","newPositionOptions"));
  on("saveNewTankBtn","click",async()=>{
    const tank={"Barcode":el("newBarcode").value.trim(),"Tank ID":el("newBarcode").value.trim(),"Gas":el("newGas").value.trim(),"Room":el("newRoom").value.trim(),"Position":el("newPosition").value.trim(),"Status":el("newStatus").value,"Updated By":el("newUpdatedBy").value.trim()};
    ...
    try{await api("addTank",{tank});showToast("New tank added.");await refreshData();el("scanResult").innerHTML=emptyState("Saved. Keep scanning or clear the scanner when done.");}
    catch(err){showToast(err.message);await refreshData();}
  });
  on("scanAgainBtn","click",()=>{el("scanResult").innerHTML="";});
}
...
async function addTankFromForm(){
  ...
  const tank={"Barcode":barcode,"Tank ID":barcode,"Gas":el("addGas").value.trim(),"Room":el("addRoom").value.trim(),"Position":el("addPosition").value.trim(),"Status":el("addStatus").value,"Updated By":el("addUpdatedBy").value.trim()||getDefaultUser()};
  if(!tank["Barcode"]||!tank["Gas"]||!tank["Room"]||!tank["Position"]){showToast("Barcode, gas, room, and position are required.");return;}
...
function clearAddForm(){
  ["addBarcode","addGas","addRoom","addPosition"].forEach(id=>{if(el(id))el(id).value="";});
